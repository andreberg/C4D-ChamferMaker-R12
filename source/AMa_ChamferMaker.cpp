...
Bool Get_H_topol_points( Modeling *krnl, PolygonObject *obj, LONG pA, LONG pB, bool &valid, LONG &wingAL, LONG &wingAR,
															bool &opened, LONG &wingBL, LONG &wingBR) ;
...
void SelectEdgesWithThrAngle( PolygonObject *obj, Neighbor *nbr, Modeling *krnl, BaseSelect *selE, 
																				Real ang, Bool andOpen) ;
...
public:
	...
	static NodeData *Alloc(void) { return gNew AMaChamMaker; }
private:
	PolygonObject *obj ;
	BaseContainer *data ;
	BaseSelect *selE ;
	...
	Bool Recurse_First_Child( BaseObject *gener, BaseThread *bt, BaseObject *op, 
...
BaseObject* AMaChamMaker::GetVirtualObjects( BaseObject *gener, HierarchyHelp *hh)
{
	...
	BaseObject *res ;
	res = gener->GetAndCheckHierarchyClone( hh, orig, HIERARCHYCLONEFLAGS_ASPOLY, &dirty, NULL,
														data->GetBool( AMa_CHMMKR_ALL_CHILDREN)) ;
	if( ! dirty)
		return res ;
	if( ! res)
		return NULL ;
...
//WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW
bool AMaChamMaker::MakeChamf_In_AMa_PARALLEL_Mode( bool CompositeModeOn, 
															bool DoubleParallel, bool CompositeFirstPass)
{
	...
	i = 0 ;
	while( selE->GetRange( i, &seg_start, &seg_end))
	{
	i++ ;
...
LONG PredPointIndInNgn( Ngon *Ngn, LONG p)
{
	...
	{
		LONG i, segStart = 0, segEnd, summ = 0 ;
...
bool GetNextPointInFan( Modeling *krnl, PolygonObject *obj, LONG pc, LONG p1, LONG p2, LONG &p3, bool &finded)
{
	LONG i, j, edgNgnCnt_c1, edgNgnCnt_c2, nextNgn ;
